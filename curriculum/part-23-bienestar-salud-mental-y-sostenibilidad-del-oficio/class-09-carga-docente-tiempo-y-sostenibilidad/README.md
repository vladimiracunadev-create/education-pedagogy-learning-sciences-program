# Clase 285 — Carga docente, tiempo y sostenibilidad

> **Parte 23 · Bienestar, salud mental y sostenibilidad del oficio** — clase 9 de 12
...
**Decisión que habilita:** determinar cuánto tiempo real exige el trabajo docente en tu establecimiento y qué tarea se elimina<br>
**Evidencia de aprendizaje:** una medición de carga docente de un mes con clasificación de tareas y propuesta de eliminación

## 🎯 Propósito

Medir la carga docente real y distinguir lo que es exigencia del oficio de lo que es acumulación evitable de tareas.
...
2. **Explicar** por qué el trabajo docente se desborda incluso cuando cada tarea parece razonable.
3. **Decidir** —cuánto tiempo real exige el trabajo docente en tu establecimiento y qué tarea se elimina— y sostener la decisión con un fundamento escrito.
4. **Producir** la evidencia de la clase —una medición de carga docente de un mes con clasificación de tareas y propuesta de eliminación— y contrastarla contra el criterio de logro.
...
|---|---|
| **Carga real** | tiempo efectivamente dedicado al trabajo, incluidas las horas no reconocidas en el contrato |
| **Trabajo invisible** | tareas que no aparecen en el horario: preparación, corrección, comunicación con familias, registros |
| **Acumulación por adición** | dinámica por la cual se agregan tareas sin retirar ninguna, hasta que el conjunto se vuelve inejecutable |
| **Tarea de bajo valor** | actividad cuyo costo en tiempo supera claramente su contribución al aprendizaje |
...
2. Clasificar la situación distinguiendo **Carga real** de **Trabajo invisible**.
3. Decidir con fundamento, usando **Acumulación por adición** como criterio y declarando la fuente.
4. Anticipar qué evidencia confirmaría la decisión y cuál la refutaría, con **Tarea de bajo valor** a la vista.
...
| **Evidencia de logro** | qué muestra una medición de carga docente de un mes con clasificación de tareas y propuesta de eliminación frente al criterio declarado de antemano |

**Frontera de aplicación.** El método vale mientras las condiciones que lo sostienen se cumplan.
La medición por autorreporte tiene error conocido y tiende a subestimar. Cuando esa condición falla, el paso siguiente no es forzar el método:
...
flowchart TB
    ...
    C --> A1["Carga real"]
    C --> A2["Trabajo invisible"]
    C --> A3["Acumulación por<br/>adición"]
    C --> A4["Tarea de bajo valor"]
    A1 & A2 & A3 & A4 --> D{{"determinar cuánto tiempo<br/>real exige el trabajo<br/>docente en tu<br/>establecimiento y qué<br/>tarea se…"}}
    D --> E["Evidencia de aprendizaje<br/>una medición de carga<br/>docente de un mes con<br/>clasificación de tareas<br/>y propuesta de<br/>eliminación"]
...
La conversación sobre carga docente suele quedarse en el reclamo general porque falta lo básico: datos. Sin medición, cada tarea nueva parece razonable por sí sola y el conjunto se vuelve inejecutable sin que nadie pueda señalar el punto de quiebre. La proporción entre horas lectivas y no lectivas está regulada, pero el trabajo invisible —corrección, comunicación, registros administrativos, plataformas— rara vez se contabiliza y es donde se produce el desborde.

### 2. Frontera conceptual: qué es y qué no es

Los cuatro conceptos de esta clase se confunden entre sí con facilidad, y esa confusión no es
inocua: produce decisiones que atacan el problema equivocado. **Carga real** no es lo
mismo que **Trabajo invisible** —tareas que no aparecen en el horario: preparación, corrección, comunicación con familias, registros—, y tratarlos como sinónimos hace
que la intervención se dirija al lugar incorrecto. Del mismo modo, **Acumulación por adición** y
**Tarea de bajo valor** describen aspectos distintos de la misma situación: el primero
dinámica por la cual se agregan tareas sin retirar ninguna, hasta que el conjunto se vuelve inejecutable, mientras el segundo actividad cuyo costo en tiempo supera claramente su contribución al aprendizaje.
...
- **Evidencia de logro.** Qué muestra una medición de carga docente de un mes con clasificación de tareas y propuesta de eliminación frente al criterio declarado de antemano. Se registra con fecha y contexto; sin eso, la señal no distingue una tendencia de una casualidad.
...
La medición es simple y poderosa: registrar durante un mes el tiempo real por tipo de tarea, incluidas las que ocurren fuera del horario. El resultado permite clasificar: qué es núcleo del oficio, qué es exigencia administrativa y qué es acumulación evitable. A partir de ahí se puede proponer eliminación concreta, que es lo único que reduce carga. Agregar bienestar sin retirar tareas es agregar una tarea más.

### 5. Qué sostiene la evidencia y qué no

La medición por autorreporte tiene error conocido y tiende a subestimar. Buena parte de la carga proviene de decisiones que exceden al establecimiento —normativa, exigencias de reporte, tamaño de curso— y que un plan local no modifica. Y existe el riesgo de que los datos se usen para exigir más eficiencia en lugar de para reducir demandas.
...
te deja en condiciones de cuánto tiempo real exige el trabajo docente en tu establecimiento y qué tarea se elimina, y esa decisión se sostiene solo si
...
|---|---|---|
| Maslach, C. & Leiter, M. (2016). *Understanding the Burnout Experience*. | sitúa la sobrecarga entre los determinantes organizacionales del desgaste. | ¿Qué supuesto de esta clase ayuda a poner a prueba? |
| OCDE. *TALIS: Estudio internacional sobre la enseñanza y el aprendizaje*. | datos comparados sobre distribución del tiempo de trabajo docente. | ¿Qué supuesto de esta clase ayuda a poner a prueba? |
...
**Paso 1 — Delimitar el contexto y comprobar —no suponer— qué saben ya los estudiantes.** El equipo escribe primero el supuesto asociado a **Carga real** y se prohíbe tratarlo como hecho. Contrasta ese supuesto con **Evidencia de partida** y anota qué parte del dato todavía no existe. Del paso sale un registro fechado y una frase explícita: «cambiaríamos de decisión si…».

**Paso 2 — Clasificar la situación distinguiendo Carga real de Trabajo invisible.** El trabajo aquí es separar lo observado de lo interpretado sobre **Trabajo invisible**. La evidencia que ordena la conversación es **Evidencia de proceso**; si su definición no está escrita, escribirla es parte del paso. Nada avanza mientras el equipo no acuerde qué contaría como refutación.

**Paso 3 — Decidir con fundamento, usando Acumulación por adición como criterio y declarando la fuente.** El riesgo de este paso es cerrar demasiado rápido alrededor de **Acumulación por adición**. Antes de concluir, se enumeran dos explicaciones alternativas del mismo patrón y se revisa si **Evidencia de logro** logra distinguirlas. Si no lo logra, hace falta otra evidencia y así debe quedar registrado.

**Paso 4 — Anticipar qué evidencia confirmaría la decisión y cuál la refutaría, con Tarea de bajo valor a la vista.** Con **Tarea de bajo valor** ya delimitado, la pregunta pasa a ser de consecuencia: qué cambia para los estudiantes, para el tiempo de clase y para la carga del equipo. **Evidencia de partida** entrega la lectura observable; el juicio profesional sigue siendo humano y debe quedar firmado por quien lo hace.
...
|---|---|---|---|
| Intervenir sobre **Carga real** | Tiempo efectivamente dedicado al trabajo, incluidas las horas no reconocidas en el contrato | Cuando **Evidencia de partida** es observable y accionable dentro del plazo de la decisión. | Sobrerreaccionar a una señal parcial. |
| Intervenir sobre **Trabajo invisible** | Tareas que no aparecen en el horario: preparación, corrección, comunicación con familias, registros | Cuando la primera explicación no distingue mecanismo ni responsable. | Convertir el concepto en etiqueta y no en intervención. |
| Observar antes de decidir | Reducir la incertidumbre antes de comprometer tiempo y credibilidad | Cuando la decisión es reversible y la evidencia disponible no distingue causas. | Observar indefinidamente y no decidir nunca. |
| Derivar o escalar | Poner la decisión donde están la competencia y la responsabilidad | Cuando hay normativa, resguardo de datos, salud o vulneración de derechos en juego. | Delegar hacia arriba lo que sí correspondía decidir en el aula. |

**Frontera de aplicación.** La medición por autorreporte tiene error conocido y tiende a subestimar. Fuera de esa frontera, la comparación
...
| Nivel | Responsabilidad sobre carga docente, tiempo y sostenibilidad |
|---|---|
...
Una medición de carga docente de un mes con clasificación de tareas y propuesta de eliminación.

Guárdala en `evidence/P23-C285-carga-docente-tiempo-y-sostenibilidad/` con estos archivos:
...
Registra tu tiempo real por tipo de tarea durante un mes. Clasifica y propone la eliminación de una tarea concreta al equipo directivo.
...
- [ ] la medición cubre un mes e incluye el trabajo realizado fuera del horario;
- [ ] la propuesta identifica tareas concretas a eliminar y no solo a optimizar;
...
- Discutir la carga sin datos y quedarse en la percepción general de agobio.
- Agregar iniciativas de bienestar sin eliminar ninguna tarea existente.
...
1. ¿Cuántas horas reales trabajaste la semana pasada, contando lo de la casa?
2. ¿Qué tarea de tu semana tiene el peor cociente entre tiempo y aprendizaje?
3. ¿Qué se eliminó la última vez que se agregó algo nuevo?
...
- OCDE. *TALIS: Estudio internacional sobre la enseñanza y el aprendizaje*. **Uso en esta clase:** datos comparados sobre distribución del tiempo de trabajo docente. Lectura selectiva: índice y capítulos pertinentes; registra edición y páginas consultadas.
...
Prepara la clase 286 y se articula con la parte 15 y con la clase 260.
...
|---|---|---|
| [← 284 · Convivencia digital, imagen y exposición](../class-08-convivencia-digital-imagen-y-exposicion/README.md) | [Parte 23](../README.md) · [Programa](../../../README.md) | [286 · Desgaste profesional y su prevención institucional →](../class-10-desgaste-profesional-y-su-prevencion-institucional/README.md) |
