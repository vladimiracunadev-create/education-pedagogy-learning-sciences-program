# Clase 229 — Cómo se compara un enfoque pedagógico

> **Parte 19 · Metodologías y enfoques pedagógicos** — clase 1 de 12
...
**Decisión que habilita:** determinar con qué criterios vas a comparar dos enfoques antes de elegir uno para tu curso<br>
**Evidencia de aprendizaje:** una matriz de comparación con criterios declarados, aplicada a dos enfoques para un mismo objetivo

## 🎯 Propósito

Establecer criterios explícitos para comparar enfoques pedagógicos, de modo que la elección deje de depender de afinidad, moda o autoridad.
...
2. **Explicar** por qué la pregunta «cuál método es mejor» no tiene respuesta y cuál sí la tiene.
3. **Decidir** —con qué criterios vas a comparar dos enfoques antes de elegir uno para tu curso— y sostener la decisión con un fundamento escrito.
4. **Producir** la evidencia de la clase —una matriz de comparación con criterios declarados, aplicada a dos enfoques para un mismo objetivo— y contrastarla contra el criterio de logro.
...
|---|---|
| **Criterio de comparación** | dimensión explícita —objetivo, conocimiento previo, tipo de contenido, costo— con la que se juzga un enfoque |
| **Efecto de reversión por experticia** | fenómeno por el cual una ayuda que beneficia al novato perjudica al que ya sabe |
| **Condición de implementación** | requisito sin el cual el método declarado no es el método que ocurre en la sala |
| **Fidelidad de implementación** | grado en que lo ejecutado coincide con lo diseñado; sin medirla no se puede juzgar el método |
...
2. Clasificar la situación distinguiendo **Criterio de comparación** de **Efecto de reversión por experticia**.
3. Decidir con fundamento, usando **Condición de implementación** como criterio y declarando la fuente.
4. Anticipar qué evidencia confirmaría la decisión y cuál la refutaría, con **Fidelidad de implementación** a la vista.
...
| **Evidencia de logro** | qué muestra una matriz de comparación con criterios declarados, aplicada a dos enfoques para un mismo objetivo frente al criterio declarado de antemano |

**Frontera de aplicación.** El método vale mientras las condiciones que lo sostienen se cumplan.
Los criterios ordenan la decisión pero no la resuelven: Cuando esa condición falla, el paso siguiente no es forzar el método:
...
flowchart TB
    ...
    C --> A1["Criterio de<br/>comparación"]
    C --> A2["Efecto de reversión<br/>por experticia"]
    C --> A3["Condición de<br/>implementación"]
    C --> A4["Fidelidad de<br/>implementación"]
    A1 & A2 & A3 & A4 --> D{{"determinar con qué<br/>criterios vas a comparar<br/>dos enfoques antes de<br/>elegir uno para tu curso"}}
    D --> E["Evidencia de aprendizaje<br/>una matriz de<br/>comparación con<br/>criterios declarados,<br/>aplicada a dos enfoques<br/>para un mismo…"]
...
La discusión sobre métodos suele plantearse entre bandos y resolverse por identidad profesional. El problema es que la pregunta está mal hecha: ningún enfoque es superior en abstracto, porque su eficacia depende del objetivo perseguido, del conocimiento previo de quien aprende y de la naturaleza del contenido. El hallazgo más útil del campo es el efecto de reversión por experticia: el mismo apoyo que hace avanzar a un novato estorba a quien ya domina el área, lo que explica buena parte de los resultados contradictorios entre estudios.

### 2. Frontera conceptual: qué es y qué no es

Los cuatro conceptos de esta clase se confunden entre sí con facilidad, y esa confusión no es
inocua: produce decisiones que atacan el problema equivocado. **Criterio de comparación** no es lo
mismo que **Efecto de reversión por experticia** —fenómeno por el cual una ayuda que beneficia al novato perjudica al que ya sabe—, y tratarlos como sinónimos hace
que la intervención se dirija al lugar incorrecto. Del mismo modo, **Condición de implementación** y
**Fidelidad de implementación** describen aspectos distintos de la misma situación: el primero
requisito sin el cual el método declarado no es el método que ocurre en la sala, mientras el segundo grado en que lo ejecutado coincide con lo diseñado;.
...
- **Evidencia de logro.** Qué muestra una matriz de comparación con criterios declarados, aplicada a dos enfoques para un mismo objetivo frente al criterio declarado de antemano. Se registra con fecha y contexto; sin eso, la señal no distingue una tendencia de una casualidad.
...
Comparar profesionalmente exige declarar los criterios antes de mirar los enfoques. Cuatro bastan para la mayoría de las decisiones: qué objetivo persigue, qué conocimiento previo supone, qué condiciones de implementación exige y cuánto cuesta sostenerlo. A eso se suma una pregunta que casi nunca se hace: cómo sabremos si funcionó. Un enfoque adoptado sin plan de verificación no puede fracasar, porque nadie definió qué sería fracasar.

### 5. Qué sostiene la evidencia y qué no

Los criterios ordenan la decisión pero no la resuelven: siguen exigiendo juicio profesional sobre un contexto que ningún estudio conoce. Además, buena parte de la investigación comparativa mide resultados de corto plazo y en tareas acotadas, que favorecen sistemáticamente a los enfoques más estructurados. Y la fidelidad de implementación es el factor que más explica los resultados y el que menos se reporta.
...
te deja en condiciones de con qué criterios vas a comparar dos enfoques antes de elegir uno para tu curso, y esa decisión se sostiene solo si
...
|---|---|---|
| Kirschner, P., Sweller, J. & Clark, R. (2006). *Why Minimal Guidance During Instruction Does Not Work*. | el argumento más fuerte contra la guía mínima; conviene leerlo junto a sus réplicas. | ¿Qué supuesto de esta clase ayuda a poner a prueba? |
| Mayer, R. (2004). *Should There Be a Three-Strikes Rule Against Pure Discovery Learning?* | separa descubrimiento puro de descubrimiento guiado, que es la distinción decisiva. | ¿Qué supuesto de esta clase ayuda a poner a prueba? |
...
**Paso 1 — Delimitar el contexto y comprobar —no suponer— qué saben ya los estudiantes.** El equipo escribe primero el supuesto asociado a **Criterio de comparación** y se prohíbe tratarlo como hecho. Contrasta ese supuesto con **Evidencia de partida** y anota qué parte del dato todavía no existe. Del paso sale un registro fechado y una frase explícita: «cambiaríamos de decisión si…».

**Paso 2 — Clasificar la situación distinguiendo Criterio de comparación de Efecto de reversión por experticia.** El trabajo aquí es separar lo observado de lo interpretado sobre **Efecto de reversión por experticia**. La evidencia que ordena la conversación es **Evidencia de proceso**; si su definición no está escrita, escribirla es parte del paso. Nada avanza mientras el equipo no acuerde qué contaría como refutación.

**Paso 3 — Decidir con fundamento, usando Condición de implementación como criterio y declarando la fuente.** El riesgo de este paso es cerrar demasiado rápido alrededor de **Condición de implementación**. Antes de concluir, se enumeran dos explicaciones alternativas del mismo patrón y se revisa si **Evidencia de logro** logra distinguirlas. Si no lo logra, hace falta otra evidencia y así debe quedar registrado.

**Paso 4 — Anticipar qué evidencia confirmaría la decisión y cuál la refutaría, con Fidelidad de implementación a la vista.** Con **Fidelidad de implementación** ya delimitado, la pregunta pasa a ser de consecuencia: qué cambia para los estudiantes, para el tiempo de clase y para la carga del equipo. **Evidencia de partida** entrega la lectura observable; el juicio profesional sigue siendo humano y debe quedar firmado por quien lo hace.
...
|---|---|---|---|
| Intervenir sobre **Criterio de comparación** | Dimensión explícita —objetivo, conocimiento previo, tipo de contenido, costo— con la que se juzga un enfoque | Cuando **Evidencia de partida** es observable y accionable dentro del plazo de la decisión. | Sobrerreaccionar a una señal parcial. |
| Intervenir sobre **Efecto de reversión por experticia** | Fenómeno por el cual una ayuda que beneficia al novato perjudica al que ya sabe | Cuando la primera explicación no distingue mecanismo ni responsable. | Convertir el concepto en etiqueta y no en intervención. |
| Observar antes de decidir | Reducir la incertidumbre antes de comprometer tiempo y credibilidad | Cuando la decisión es reversible y la evidencia disponible no distingue causas. | Observar indefinidamente y no decidir nunca. |
| Derivar o escalar | Poner la decisión donde están la competencia y la responsabilidad | Cuando hay normativa, resguardo de datos, salud o vulneración de derechos en juego. | Delegar hacia arriba lo que sí correspondía decidir en el aula. |

**Frontera de aplicación.** Los criterios ordenan la decisión pero no la resuelven: Fuera de esa frontera, la comparación
...
| Nivel | Responsabilidad sobre cómo se compara un enfoque pedagógico |
|---|---|
...
Una matriz de comparación con criterios declarados, aplicada a dos enfoques para un mismo objetivo.

Guárdala en `evidence/P19-C229-como-se-compara-un-enfoque-pedagogico/` con estos archivos:
...
Elige un objetivo concreto de tu asignatura y compara dos enfoques con cuatro criterios declarados. Escribe cuál eliges, qué condición exige y cómo verificarás el resultado.
...
- [ ] la matriz declara los criterios antes de aplicarlos y los usa igual con ambos enfoques;
- [ ] la comparación incluye condiciones de implementación y plan de verificación;
...
- Comparar enfoques por sus resultados sin verificar si se implementaron como se diseñaron.
- Elegir método por identidad profesional y buscar después la evidencia que lo respalde.
...
Los criterios explícitos protegen a los estudiantes con menos conocimiento previo, que son los primeros perjudicados cuando un enfoque de guía baja se adopta porque suena moderno. Declarar el supuesto de conocimiento previo es también declarar a quién dejaría fuera.
...
1. ¿Qué objetivo específico persigues, y qué enfoque tendría sentido si el objetivo fuera otro?
2. ¿Qué conocimiento previo tienen realmente tus estudiantes sobre este contenido?
3. ¿Qué observarías dentro de seis semanas que te haría concluir que el enfoque no funcionó?
...
Abre la parte 19 y ordena las once clases siguientes. Retoma la clase 121 y prepara la 240.
...
|---|---|---|
| [← Parte 19](../README.md) | [Parte 19](../README.md) · [Programa](../../../README.md) | [230 · Instrucción explícita y enseñanza directa →](../class-02-instruccion-explicita-y-ensenanza-directa/README.md) |
